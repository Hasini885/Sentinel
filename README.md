...
```
        ...
        │  Groq Llama 3.3 70B, strict JSON:     │
...
**Stack:** FastAPI + SQLAlchemy + Alembic + Postgres, Redis Streams, Groq Llama 3.3 70B (free tier) for risk scoring, Next.js 14 (App Router) + TypeScript + Tailwind + Recharts, DeepEval for the risk-scorer regression suite.
...
- Python 3.11+
- Node 18+
- A Groq API key (free — get one at https://console.groq.com/keys, no card required)
...
Set `GROQ_API_KEY` in `.env`. The other defaults work as-is. `.env` is gitignored.
...
`@intercept_action(agent_name=..., action_type=...)` wraps an agent's tool function. When the tool is called, the decorator intercepts it **before execution** and sends the action type and payload to Groq's Llama 3.3 70B.

The scorer requests a JSON object (Groq's OpenAI-compatible `chat.completions` with `response_format=json_object`) and validates it against a Pydantic schema, so a malformed response fails loudly rather than being parsed hopefully. It returns three things in one call:
...
**The scorer fails closed.** If the Groq call errors, the action is graded `high` with the error as its reason, so the policy engine blocks or holds it rather than letting an unclassified action through. An API outage stops the agent; that's the correct trade for a governance tool, but it is a trade.

Token usage from the scoring call is real (`response.usage`). The action's *own* token cost is a rough `len(text)/4` heuristic. Both are priced at Groq list rates and summed into `tokens_used` / `estimated_cost_usd`. On the free tier your actual spend is $0 — the dashboard shows the list-price equivalent so the ROI and auto-downgrade numbers stay meaningful.
...
The suite skips cleanly (rather than failing) when `GROQ_API_KEY` is unset.
